...
|---|---|---|:---:|---|---|---|
| 1 | L1.1 Introduction to transformer architecture; L1.2 Attention is all you need; L1.3 Self-Attention; L1.4 Multi-headed attention | Transformer motivation and anatomy; Q/K/V; scaled dot-product attention; self-attention matrices; multi-head shapes; concatenation/output projection; parameter counting; encoder conventions | A | Official slides + attention/counting supplements + multiple personal Q1 guides | Previously studied; mastery unassessed. GA1 solution and four PYQs available. | Closed-book shape/parameter diagnostic, then build canonical module around errors. |
| 2 | L2.1 Teacher Forcing and Masked attention; L2.2 Zooming into Decoder layer; L2.3 Positional Encoding; L2.4 Sinusoidal encoding; L2.5 Batch normalization; L2.6 Layer normalization | Teacher forcing; causal mask; decoder self-attention vs cross-attention; positional signals; sinusoidal PE; BatchNorm vs LayerNorm; Add & Norm; mask counting and LayerNorm numerics | A | Official slides + personal Q1 deep/speed/prerequisite packs | Previously studied; mastery unassessed. GA2 solution and PYQs available. | Diagnostic on mask entries, decoder flow, PE values, and normalization axes. |
| 3 | L3.1 Introduction to language modelling - Motivation; L3.2 Language Modelling; L3.3 Transformers for language modelling; L3.4 Causal Language Modelling; L3.5 Generative Pre-trained Transformer; L3.6 Pre-training and Fine Tuning | Chain rule and autoregressive factorization; language modelling; causal LM; GPT architecture/masking; pre-training vs fine-tuning; likelihood/loss interpretation | A | Official slides + prerequisite/math/deep personal notes | Previously studied; mastery unassessed. GA3 solution and PYQs available. | Link probability foundations to transformer CLM with one complete toy-sequence loss walk-through. |
| 4 | L4.1 Decoding Strategies; L4.2 Exhaustive search; L4.3 Gready and Beam Search; L4.4 Sampling Stratagies: Top-K; L4.5 Sampling Stratagies: Top-P; L4.6 BERT - Masked Language Modelling; L4.7 Next Sentence Prediction; L4.8 Pre-training and Parameters; L4.9 Adapting to Downstream tasks | Exhaustive/greedy/beam decoding; top-k and top-p sampling/renormalization; BERT encoder; MLM and NSP; pre-training parameters; downstream adaptation; BERT vs GPT | A | Official two-part slides + personal Q1 packs + published comparison guides | Previously studied; mastery unassessed. GA4 solution and PYQs available. | Run a decoding tree by hand and a model/objective reverse-lookup test. |
...
| 1 | Course Introduction; History (optional); Image Formation (optional); Image Representation; Linear Filtering; Image in Frequency Domain; Image Sampling | Images as arrays/signals; sampling; correlation vs convolution; kernel extraction; separability; low/high pass; median/average filters; contrast; Laplacian/LoG; integral image; operation counts | A | Full personal Quiz 1 pack + Week 1 recitation HTML + convolution playground + official seven-part deck | Previously studied; mastery unassessed. Historical GA1 score 86. | Run filtering-algebra and convolution-playground diagnostic; verify optional vs examinable boundaries. |
| 2 | Edge Detection; From Edges to Blobs and Corners; Scale Space, Image Pyramids and Filter Banks; Feature Detectors: SIFT and Variants; Image Segmentation; Other Feature Spaces; Human Visual System | Causes/uses of edges; smoothing and gradients; Canny pipeline; second moment matrix; edge/corner eigenvalue interpretation; straightness; scale space/pyramids; SIFT; segmentation/features | A | Full personal Quiz 1 pack + official seven-part deck | Previously studied; mastery unassessed. Historical GA2 score 100. | Maintenance diagnostic on Canny order, corner matrix, straightness, and SIFT invariance. |
| 3 | Neural Networks review; Feedforward Neural Networks and Backpropagation; Gradient Descent and Variants; Regularization; Improving Training | Activations and derivatives; forward/backprop; softmax; perceptron; GD/SGD/momentum/adaptive optimizers; initialization; regularization; hyperparameter tuning | A | Full personal Quiz 1 pack + official five-part slide deck | Previously studied; mastery unassessed. GA3 key metadata exists but no submitted score. | Closed-book derivative/optimizer update drill and one full backprop calculation. |
| 4 | CNN Introduction Parts 1–2; Backpropagation in CNNs; AlexNet and VGG | Convolution-layer output sizes; parameters and compute; CNN backprop gradient shapes; receptive field; pooling/layer facts; AlexNet/VGG signatures | A | Full personal Quiz 1 pack + official three-part deck | Previously studied; **needs practice** from historical GA4 score 64. | Prioritize output/parameter/receptive-field numerics and architecture signatures. |
...
|---|---|---|---|---|---|---|
| LLM Quiz 1 (W1–4) | Complete | Shared RNN/Seq2Seq prerequisites integrated; transformer material pending | Shared Volume 0 complete; LLM W1–4 volumes pending | Previously studied; mastery unassessed | GA1–4 solutions + 4 PYQs available | Take Volume 0 test, then build Transformer Foundations |
...
| DLCV Quiz 1 (W1–4) | Complete | Strong full pack; Week 3 foundation integrated into Shared Volume 0 | Shared Volume 0 complete; Quiz 1 recovery volume pending | Previously studied; mastery unassessed | Historical GA weakness at W4 | Use Volume 0 for W3, then prioritize W4 numerics in the recovery volume |
...
2. **Next:** Transformer Foundations - LLM Weeks 1–2, reused by DLCV Weeks 8–9.
3. DLCV Quiz 1 Recovery - Weeks 1–4, with the shared Week 3 foundation linked instead of duplicated.
4. Language Models and Decoding - LLM Weeks 3–4.
5. LLM Week 8 companion for the existing Week 7 module.
...
Open Shared Volume 0 through the hub, complete its timed pulses and final test, and return the copied report. The score and error categories will adjust practice depth, but will not remove any concept from later material.

No mastery has been inferred from the existence of old notes or from generating Volume 0.
