...
8. **Next:** Efficient and Vision Transformers - LLM Weeks 11–12 plus DLCV Week 9.
...
Open `../dlcv/volume-07-sequence-attention-vision-interactive.html`. Do the 90-second pulse before reading, write every LSTM/GRU gate preactivation before revealing the official PA7/GA7 calculation, and complete the attention score pipeline by hand. Then use `../dlcv/visual-attention-playground.html` to reproduce the PA8 and GA8 spatial contexts, vary temperature, and explain soft versus hard attention without option-memory. Finish the 10-item objective test and written questions 21–30.
