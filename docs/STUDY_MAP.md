...
| 11 | L11.1 Motivation for fast attention mechanism, time and space complexity of attention; L11.2 Fast attention Mechanisms: Approaches; L11.3 Local attention mechanisms; L11.4 Sparse Block Attention; L11.5 Low rank approximation; L11.6 Fast Inference Mechanisms | Exact quadratic FLOPs/memory; local/dilated/global/random/block patterns; Linformer/Performer; FlashAttention; KV cache; MHA/MQA/GQA | A | **Shared Volume 8 + dedicated playground complete**; 101 official pages, searchable Markdown, GA11, and four PYQs integrated | Not recorded. GA11 has no submitted answers; derived results are explicitly tied to official conventions and direct PYQ evidence. | Re-derive GA11 FLOPs, memory, block masks, and KV cache; classify each fast method by what it changes before using the attention graph/cache labs. |
| 12 | L12.1 Motivation for Relative Position Encoding; L12.2 Relative Position Encoding; L12.3 Rotary Positional Embedding; L12.4 ALiBi and NoPE | APE limits; Shaw/Transformer-XL/T5 RPE; clipping; RoPE rotation proof; ALiBi score bias; NoPE; parameters/extrapolation/insertion point | A | **Shared Volume 8 + positional playground complete**; 41 official pages, searchable Markdown, GA12, and four PYQs integrated | Not recorded. GA12 has no submitted answers; direct repeated PYQ keys resolve high-value patterns. | Draw the insertion point for every scheme, reproduce the RoPE relative-dot-product proof and ALiBi bias, and explain why inference-only PE removal is invalid. |
...
| Week | Official lectures | Core concepts and exam skills | Tier | Material status | Study/practice status | Next evidence-based action |
|---|---|---|:---:|---|---|---|
| 5 | Evolution of CNN Architectures: InceptionNet, ResNet; Newer and Recent CNN Architectures; Finetuning CNNs; Visualizing CNNs | Inception/ResNet/EfficientNet and architecture signatures; standard vs depthwise separable convolution; parameter/compute calculations; fine-tuning; visualization, CAM/GAP | A | **Volume 6 complete**; Quiz 2 quick-study note, official four-part deck, GA5/PA5, and PYQs integrated | Previously studied; historical GA5 score 78; mastery unassessed | Complete Volume 6 architecture/cost drills and record error tags. |
| 6 | Object Detection before/deep learning; Two-Stage Models; Single-stage Models; Segmentation | Detection pipeline; proposals/anchors; IoU; Faster R-CNN losses/counts; two-stage vs single-stage; NMS where covered; mAP/11-point interpolation; segmentation distinctions | A | **Volume 6 + Detection Metrics Playground complete**; personal note, official three-part deck, GA6/PA6, and PYQs integrated | Previously studied; no submitted GA6 score; mastery unassessed | Solve IoU, NMS, anchor/loss, AP/mAP, and Dice blocks under timed conditions. |
| 7 | RNN Introduction; Backpropagation in RNNs; LSTMs and GRUs; Video Understanding using CNNs and RNNs | RNN recurrence and parameter counts; BPTT; vanishing/exploding gradients; LSTM/GRU components; architecture-to-application mapping; video models | A | **Volume 7 complete**; Shared Volume 0 linked; all four official decks, GA7/PA7, PYQs, and personal calculations integrated | Previously studied; **needs practice** from historical GA7 score 50 | Drill exact gate preactivations and state updates until PA7/GA7 can be derived closed-book; do not memorize only rounded outputs. |
| 8 | Attention in Vision Models; Soft and Hard Attention: Image Captioning; Beyond Captioning: Visual QA and Dialog; Self-Attention and Transformers | Visual attention/alignment scores; soft vs hard attention; image captioning; VQA/dialog; attention numerics; self-attention and transformer claims | A | **Volume 7 + Visual Attention Playground complete**; Shared Volume 1 linked; four official decks, GA8/PA8, PYQs, and personal note integrated | Previously studied; historical GA8 score 93; maintenance | Reproduce both spatial-context presets and the score-softmax-context block, then explain changing PYQ statements from mechanism and wording. |
...
| LLM Quiz 2 (W5–8) | Complete | Strong master pack | W5, W7, and W8 interactives complete; W6 N/A; Data Pipeline Playground complete | Previously studied; mastery unassessed | GA5/7/8 solutions and PYQs embedded | Test each module closed-book and retest only recorded error categories |
...
| DLCV Quiz 2 (W5–8) | Complete | Complete Quiz 2 pack integrated; Shared Volumes 0–1 linked for common RNN/Transformer foundations | Volumes 6–7 plus Detection Metrics and Visual Attention Playgrounds complete | Previously studied; mastery unassessed | GA5–8, PA5–8, four supplied PYQs, and historical error patterns embedded; W7 remains priority | Study Volume 7 closed-book; repeat exact LSTM/GRU calculations until preactivations and values are recalled without reveal, then complete the visual-attention drills |
...
- **2026-08-30:** Generated DLCV Volume 7 from all 523 official Week 7–8 slide pages, GA7/PA7, GA8/PA8, four supplied PYQs, and the personal Quiz 2 pack; added exact course-convention RNN/LSTM/GRU remediation and a dedicated spatial/temporal Visual Attention Playground.
